...
import { deplacerColis, rechercherColisParNumero } from '@/lib/colis-actions'
...
export default function DeplacementPage() {
  ...
  async function onVerifier() {
    ...
    if (!colis) {
      setColisTrouve(null)
      setMessage({ type: 'error', texte: `Colis "${numeroColis}" introuvable.` })
      return
    }
  ...
  async function onConfirmer() {
    ...
    const result = await deplacerColis({ numeroColis, nouvelEmplacement, utilisateurId })
  ...
  return (
    <main className="p-4 max-w-md mx-auto">
      ...
      {colisTrouve && (
        <div className="mt-4 p-4 rounded-xl border border-[#D9D2C4] bg-white flex flex-col gap-3">
          <div>
          ...
          {nouvelEmplacement && (
            <p className="text-sm text-[#1A1A1A]">
              {colisTrouve.emplacement} → <span className="font-semibold">{nouvelEmplacement}</span>
            </p>
          )}

          <button
            onClick={onConfirmer}
            disabled={!nouvelEmplacement}
            className="py-3.5 rounded-xl bg-[#E8703A] text-white font-semibold text-base shadow-sm active:scale-[0.98] transition disabled:opacity-40"
          >
